...
module A2ADocs
  module MarkdownExport
    ...
    def resolved_markdown(item, payload)
      ...
      Jekyll.logger.warn('a2a-docs', "Markdown export failed for #{relative_path}: #{e.message}")
      raw
    end
...
capture_markdown = lambda do |item, payload|
  next unless A2ADocs::MarkdownExport.enabled?(item)

  raw = A2ADocs::MarkdownExport.resolved_markdown(item, payload)
  item.data['_raw_markdown'] = A2ADocs::MarkdownExport.with_title(raw, item.data['title'])
end
...
Jekyll::Hooks.register :site, :post_write do |site|
  A2ADocs::MarkdownExport.write_files(site)
end
