...
__END__
  describe "A2A::Server::Middleware::LimitHistoryLength" do
    it "defaults to server max when client does not specify" do
      ...
      mw = A2A::Server::Middleware::LimitHistoryLength.new(downstream, 20)
    ...
    it "always returns an integer" do
      ...
      downstream = -> (env) { env["a2a.history_length"] }
      mw = A2A::Server::Middleware::LimitHistoryLength.new(downstream, 10)
      env = { "a2a.request" => request }
